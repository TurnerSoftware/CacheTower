...
{
	[TestClass]
	public class CacheStackContextSynchronousTests : TestBase
	{
		...
		{
			using var cacheStack = new CacheStack<object>(() => null, new[] { new MemoryCacheLayer() }, Array.Empty<ICacheExtension>());
		...
		{
			using var cacheStack = new CacheStack<int>(() => 123, new[] { new MemoryCacheLayer() }, Array.Empty<ICacheExtension>());
